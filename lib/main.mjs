...
import { additionalContentStyles } from './styles';
import fs                          from 'fs';
...
let thisDirName  = '';
if (isWebpack) {
    thisFileName = path.normalize(path.join(__dirname, '../index.js'));
...
async function main() {
    ...
    try {
        ...
        await app.exposeFunction('renderByMenneu', async (source, data, options, srcPath, ...exportPath) => {
            ...
            if (options.outputFormat.toLowerCase() === 'html' &&
                    outPath.startsWith(path.normalize(path.join(thisDirName,'./contents/out/preview')))) {
